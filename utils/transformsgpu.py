...
# cut the source part and paste them onto target
def oneMix(mask, data = None, target = None):
    #Mix
    if not (data is None):
        # print('mask[0].shape', mask[0].shape) # torch.Size([512, 512])
        # print('data[0].shape', data[0].shape) # torch.Size([3, 512, 512])
        stackedMask0, _ = torch.broadcast_tensors(mask[0], data[0]) 
        # print('stackedMask0.shape', stackedMask0.shape) # torch.Size([3, 512, 512])
        data = (stackedMask0*data[0]+(1-stackedMask0)*data[1]).unsqueeze(0)
        # print('data.shape', data.shape) #  torch.Size([1, 3, 512, 512])
    if not (target is None):
        # print('mask[0].shape', mask[0].shape)     #  torch.Size([512, 512])
        # print('target[0].shape', target[0].shape) #  torch.Size([512, 512])
        stackedMask0, _ = torch.broadcast_tensors(mask[0], target[0])
        # print('stackedMask0.shape', stackedMask0.shape) # torch.Size([512, 512])
        target = (stackedMask0*target[0]+(1-stackedMask0)*target[1]).unsqueeze(0)
        # print('target.shape', target.shape)       #  torch.Size([1, 512, 512])
    return data, target

# cut the targer part and paste them onto source
def oneMix_inverse(mask, data = None, target = None):
    #Mix # data[0] is source; data[1] is target; 
    if not (data is None):
        # print('mask[0].shape', mask[0].shape) # torch.Size([512, 512])
        # print('data[0].shape', data[0].shape) # torch.Size([3, 512, 512])
