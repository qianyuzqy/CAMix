...
def main():
    ...
    print('gtav size:',len(trainloader))
    #--------------------------------------------------------------------------------------
    ...
    for i_iter in range(start_iteration, num_iterations):
        ...
        if train_unlabeled:
            ...
            spatial_matrix = spr_remain                   # torch.Size([bs, 19, 512, 512])
            ...
            if i_iter < warm_up_iter:
                ...
                # Generate mask for inverse DACS(ClassMix)
                if mix_mask == "inverse_classmix":
                    for image_i in range(batch_size):
                        # Get all semantic categories that appear in each image
            ...
            # print('images.shape', images.shape)               # torch.Size([2, 3, 512, 512])
            # print('images_remain.shape', images_remain.shape) # torch.Size([2, 3, 512, 512])

            # Warm up using ClassMix
            if i_iter < warm_up_iter:
                # Mix up the input images of source and target
                inputs_u_s0, _ = strongTransform(strong_parameters, data = torch.cat((images[0].unsqueeze(0),images_remain[0].unsqueeze(0)))) v    
            ...
            # Normal Training with CAMix
                ...
                # Mix up the input images of source and target
                inputs_u_s0, _ = strongTransform_inverse(strong_parameters, data = torch.cat((images[0].unsqueeze(0),images_remain[0].unsqueeze(0)))) # torch.Size([1, 3, 512, 512])
                strong_parameters["Mix"] = MixMask1
        ...
        if i_iter % val_per_iter == 0 and i_iter != 0:
            model.eval()
            if dataset == 'cityscapes':
                mIoU, _, _ = evaluate(model, dataset, source_data=source_data, i_iter=i_iter, ignore_label=255, input_size=(512,1024), save_dir=checkpoint_dir)
                mIoU = mIoU * 100
                print('Cur mIoU', mIoU)
    ...
    model.eval()
    if dataset == 'cityscapes':
        mIoU, _, _ = evaluate(model, dataset, source_data=source_data, i_iter=num_iterations, ignore_label=255, input_size=(512,1024), save_dir=checkpoint_dir)
        mIoU = mIoU * 100
        print('Cur mIoU', mIoU)
        print('Cur mIoU iter', i_iter)
        f_mIoU = open(osp.join(checkpoint_dir, 'mIoU.txt'), 'a')
