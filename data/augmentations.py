...
class CCompose(object):
    ...
    def __call__(self, img, mask,centroid):
        ...
        for a in self.augmentations:
            img, mask, position = a(img, mask, centroid) #gqq
        return np.array(img), np.array(mask, dtype=np.uint8), position #gqq
...
class Compose(object):
    ...
    def __call__(self, img, mask):
        ...
        assert img.size == mask.size
        for a in self.augmentations:
            img, mask, position = a(img, mask) #gqq
        return np.array(img), np.array(mask, dtype=np.uint8), position #gqq
...
class RandomCrop_city(object):  # used for results in the CVPR-19 submission
    ...
    def __call__(self, img, mask):
        ...
        #print('img.size', img.size, 'mask.size', mask.size)
        assert img.size == mask.size    # (2048, 1024)
        w, h = img.size
        th, tw = self.size
...
class RandomCrop_gta(object):  # used for results in the CVPR-19 submission
    ...
    def __call__(self, img, mask):
        ...
        assert img.size == mask.size
        # print('img.size', img.size)   #  w,h (1280, 720)
        # print('self.size', self.size) #  h,w (512,1024)
        w, h = img.size
        th, tw = self.size
