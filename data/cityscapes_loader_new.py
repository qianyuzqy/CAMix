...
from utils.random_crop import RandomCrop_spa, RandomCrop_img
...
class cityscapesLoader(data.Dataset):
    ...
    def __init__(
        ...
        self.images_base = os.path.join(self.root, "leftImg8bit","leftImg8bit", self.split)
        self.annotations_base = os.path.join(
            self.root, "gtFine", "gtFine", self.split
        )
    ...
    def __getitem__(self, index):
        ...
        #     print('lbl.shape after aug', lbl.shape)

        if self.is_transform:
            img, lbl = self.transform(img, lbl)

        if self.augmentations is not None:
            # img, lbl = self.augmentations(img, lbl) # original code
            img, lbl, spatial_matrix = self.augmentations(img, lbl)
            print('img.shape after aug', img.shape)
            print('lbl.shape after aug', lbl.shape)
            print('spatial_matrix.shape after aug', spatial_matrix.shape)
        ...
        if self.return_id:
            return img, lbl, spatial_matrix, img_name, img_name, index
        return img, lbl, spatial_matrix, img_path, lbl_path, img_name
